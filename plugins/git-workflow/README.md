...
└── jira.md    # Jira 티켓 조회(스킬 → 브라우저 → MCP 3단계)·검토 게이트·상태 전환 프로토콜
...
1. **Jira 티켓 조회** — 조회 스킬 → 브라우저 세션(`chrome-devtools`) → 공식 Atlassian MCP 순으로 내려가며 본문·댓글·첨부를 수집. MCP까지 내려가면 첨부는 사용자에게 요청. 상태 전환은 언제나 MCP
